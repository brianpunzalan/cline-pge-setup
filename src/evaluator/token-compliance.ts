...
export function checkColorTokens(files: GeneratedFile[]): CriterionResult {
  ...
  return { id: "color_tokens", score, passed: score >= 0.7, violations, details: `Found ${violations.length} hardcoded color value(s).` };
}
...
export function checkSpacingTokens(files: GeneratedFile[]): CriterionResult {
  ...
  for (const file of files) {
    ...
    for (const m of countMatches(text, INLINE_STYLE_SPACING)) {
      violations.push({ file: file.path, line: lineOf(text, m.index ?? 0), message: `Inline style spacing override: "${m[0].trim()}"`, suggestion: "Use DS Stack/Grid gap props or spacing tokens" });
    }
  }

  const score = Math.max(0, 1 - violations.length * 0.1);
  return { id: "spacing_tokens", score, passed: score >= 0.7, violations, details: `Found ${violations.length} hardcoded spacing value(s).` };
}
...
export function checkTypographyTokens(files: GeneratedFile[]): CriterionResult {
  ...
  const score = Math.max(0, 1 - violations.length * 0.15);
  return { id: "typography_tokens", score, passed: score >= 0.7, violations, details: `Found ${violations.length} raw typography value(s).` };
}
...
export function checkShadowTokens(files: GeneratedFile[]): CriterionResult {
  ...
  return { id: "shadow_tokens", score, passed: true, violations, details: `Found ${violations.length} raw box-shadow value(s).` };
}
