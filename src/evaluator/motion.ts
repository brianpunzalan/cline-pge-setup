...
// Raw timing values embedded in transition/animation shorthand (not prefixed by var()
const RAW_TRANSITION_TIMING = /transition\s*:[^;}\n]*?(?<!\bvar\([^)]*)\b(\d+(?:\.\d+)?(?:ms|s))\b/g;
const RAW_ANIMATION_TIMING  = /animation(?:-duration)?\s*:[^;}\n]*?(?<!\bvar\([^)]*)\b(\d+(?:\.\d+)?(?:ms|s))\b/g;

// Explicit longhand timing properties without var()
const RAW_DURATION = /(?:animation-duration|transition-duration)\s*:\s*(?!var\()(\d+(?:\.\d+)?(?:ms|s))/g;

// @keyframes definition (new animation being added)
...
export function checkMotionTokens(files: GeneratedFile[]): CriterionResult {
    ...
    for (const [pattern, prop] of [
      ...
      for (const match of countMatches(text, pattern)) {
        ...
        violations.push({
          ...
          message: `Raw ${prop} value: "${value}" — use a token`,
          value,
          suggestion: "Use var(--duration-fast/normal/slow) and var(--easing-*) tokens",
        });
      }
    }
  }
  ...
  return {
    ...
    violations,
    details: `Found ${violations.length} raw motion value(s).`,
  };
}
...
export function checkReducedMotionGuard(files: GeneratedFile[]): CriterionResult {
      ...
      if (hasGuard) {
        ...
        for (const match of newAnimations) {
          violations.push({
            file: file.path,
            line: lineOf(text, match.index ?? 0),
            message: `Animation "${match[0]}" missing @media (prefers-reduced-motion) guard`,
            suggestion: "Wrap animation in @media (prefers-reduced-motion: no-preference) { ... }",
          });
        }
      }
    }
  ...
  const score = totalNewAnimations === 0
    ? 1.0
    : guardedAnimations / totalNewAnimations;
  ...
  return {
    ...
    passed: true, // threshold is 0
    violations,
    details: `${guardedAnimations}/${totalNewAnimations} new animation(s) have a prefers-reduced-motion guard.`,
  };
}
