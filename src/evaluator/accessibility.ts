  ...
  for (const file of files) {
    ...
    for (const match of countMatches(text, CLICK_ON_NON_INTERACTIVE)) {
      ...
      if (!HAS_ROLE.test(element) && !HAS_TAB_INDEX.test(element)) {
        violations.push({ file: file.path, line: lineOf(text, match.index ?? 0), message: `onClick on <${match[1]}> without role or tabIndex — not keyboard accessible`, value: element.slice(0, 80), suggestion: 'Add role="button" and tabIndex={0}, or use a DS <Button> component instead' });
      }
    }
  }
  return { id: "focus_management", score: violations.length === 0 ? 1.0 : 0.0, passed: violations.length === 0, violations };
}
...
export function checkAxeViolations(files: GeneratedFile[]): CriterionResult {
  ...
  for (const file of files) {
    ...
    for (const match of countMatches(text, MISSING_ALT)) {
      violations.push({ file: file.path, line: lineOf(text, match.index ?? 0), message: "img element missing alt attribute", suggestion: 'Add alt="" for decorative images or alt="description" for informational ones' });
    }
    for (const match of countMatches(text, MISSING_LABEL)) {
      violations.push({ file: file.path, line: lineOf(text, match.index ?? 0), message: "input element missing accessible label", suggestion: "Add aria-label, aria-labelledby, or associate with a <label> via id" });
    }
    for (const match of countMatches(text, EMPTY_BUTTON)) {
      violations.push({ file: file.path, line: lineOf(text, match.index ?? 0), message: "Empty button element — no accessible text", suggestion: "Add text content or aria-label to the button" });
    }
    ...
    if (headingMatches.length > 1) {
      ...
      for (let i = 1; i < levels.length; i++) {
        if (levels[i]! - levels[i - 1]! > 1) {
          violations.push({ file: file.path, line: lineOf(text, headingMatches[i]!.index ?? 0), message: `Heading level skip: h${levels[i - 1]} → h${levels[i]}`, suggestion: "Do not skip heading levels — use sequential h1, h2, h3..." });
        }
      }
    }
  }
  const score = Math.max(0, 1 - violations.length * 0.1);
  return { id: "axe_violations", score, passed: score >= 0.7, violations, details: `Found ${violations.length} accessibility violation(s) via static analysis.` };
}

interface ColorPair { fg: string; bg: string; context: string; file: string; }
...
function relativeLuminance([r, g, b]: [number, number, number]): number {
  const srgb = [r, g, b].map(c => { const s = c / 255; return s <= 0.04045 ? s / 12.92 : Math.pow((s + 0.055) / 1.055, 2.4); }) as [number, number, number];
...
export function checkContrastRatio(_files: GeneratedFile[]): CriterionResult {
  ...
  for (const pair of TEXT_BG_PAIRS) {
    const fgHex = TOKEN_HEX.get(pair.fg); const bgHex = TOKEN_HEX.get(pair.bg);
    if (!fgHex || !bgHex) continue;
    const ratio = wcagContrast(fgHex, bgHex);
    if (ratio < 4.5) {
      violations.push({ file: pair.file, message: `Contrast ratio ${ratio.toFixed(2)}:1 for "${pair.context}" — fails WCAG AA (requires 4.5:1)`, value: `${pair.fg} on ${pair.bg}`, suggestion: "Use a darker foreground or lighter background token to meet 4.5:1" });
    }
  }
  const score = 1 - violations.length / Math.max(TEXT_BG_PAIRS.length, 1);
  return { id: "contrast_ratio", score: Math.max(0, score), passed: score >= 0.9, violations, details: `Checked ${TEXT_BG_PAIRS.length} color pair(s); ${violations.length} failed WCAG AA.` };
}
