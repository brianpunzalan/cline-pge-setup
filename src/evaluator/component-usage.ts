...
import {
  DS_COMPONENT_MAP,
  DS_LAYOUT_COMPONENTS,
  DS_COMPONENT_SCHEMAS,
  BANNED_DS_PROPS,
  CLASSNAME_ALLOWED_COMPONENTS,
} from "../config/ds-components.js";

// ── Patterns ──────────────────────────────────────────────────────────────────

// Matches native JSX elements: <button, <input, <select, etc.
function nativeElementPattern(tag: string) {
  return new RegExp(`<${tag}(?:\\s|>|/)`, "g");
}

// Matches DS component JSX with props: <Button size="tiny"
function dsComponentWithProps(componentName: string) {
  return new RegExp(`<${componentName}\\s([^>]+?)(?:/>|>)`, "gs");
}

// Matches inline style override on any element
const INLINE_STYLE = /<\w+[^>]+\bstyle=\{/g;

// Matches className injection on DS components (not layout primitives)
function classNameOnComponent(componentName: string) {
  return new RegExp(`<${componentName}[^>]+\\bclassName=`, "g");
}
    ...
    for (const tag of nativeTags) {
      ...
      for (const match of matches) {
        ...
        violations.push({
          ...
          message: `Native <${tag}> used — design system equivalent exists`,
          value: match[0].trim(),
          suggestion: `Replace with DS component: ${dsAlternatives}`,
        });
      }
    }
  }

  const score = totalNative === 0
    ? 1.0
    : 1 - shouldBeDS / totalNative;

  return {
    id: "native_override_rate",
    score: Math.max(0, score),
    passed: score >= 0.8,
    violations,
    details: `${shouldBeDS}/${totalNative} native elements should be DS components.`,
  };
}

// ── DS Component Prop Correctness ─────────────────────────────────────────────
...
export function checkPropCorrectness(files: GeneratedFile[]): CriterionResult {
  ...
  for (const file of files) {
    ...
    for (const [componentName, schema] of Object.entries(DS_COMPONENT_SCHEMAS)) {
      ...
      for (const match of matches) {
        ...
        for (const [prop, validValues] of Object.entries(schema)) {
          ...
          if (!validValues.includes(usedValue)) {
            violations.push({
              file: file.path,
              line: lineOf(text, match.index ?? 0),
              message: `<${componentName} ${prop}="${usedValue}"> — invalid value`,
              value: usedValue,
        ...
        // Check for banned props (style, sx, css)
        for (const banned of BANNED_DS_PROPS) {
          if (new RegExp(`\\b${banned}=\\{`).test(propsStr)) {
            violations.push({
              file: file.path,
              line: lineOf(text, match.index ?? 0),
              message: `<${componentName}> uses banned prop "${banned}" — do not override DS component styles`,
              suggestion: `Remove the "${banned}" prop; use DS tokens or variant props instead`,
            });
          }
        }

        // className is only allowed on layout primitives
        if (/\bclassName=/.test(propsStr) && !CLASSNAME_ALLOWED_COMPONENTS.has(componentName)) {
          violations.push({
            file: file.path,
            line: lineOf(text, match.index ?? 0),
            message: `<${componentName} className="..."> — className injection not allowed on DS components`,
            suggestion: "Remove className; use variant/size props or DS tokens instead",
          });
        }
      }
    }

    // Also flag any inline style on DS components
    for (const match of countMatches(text, INLINE_STYLE)) {
      violations.push({
        file: file.path,
        line: lineOf(text, match.index ?? 0),
        message: `Inline style override detected: "${match[0].slice(0, 60)}"`,
        suggestion: "Use DS token CSS variables or component variant props",
      });
    }
  }
  ...
  return {
    ...
    passed: score >= 0.8,
    violations,
    details: `Found ${violations.length} prop violation(s).`,
  };
}
...
export function checkLayoutPrimitives(files: GeneratedFile[]): CriterionResult {
    ...
    if (file.language === "css" || file.language === "scss") {
      ...
      for (const match of [...flexMatches, ...gridMatches]) {
        violations.push({
          file: file.path,
          line: lineOf(text, match.index ?? 0),
          message: `Raw ${match[0].trim()} — use DS layout primitive instead`,
          suggestion: "Replace with <Stack>, <Grid>, <Cluster>, or <Sidebar>",
        });
      }
    }
  ...
  return {
    ...
    violations,
    details: `${dsLayouts} DS layout component(s) vs ${rawLayouts} raw flex/grid.`,
  };
}
...
export function checkThemeProviderScope(files: GeneratedFile[]): CriterionResult {
  ...
  // Check that at least one file wraps output in ThemeProvider
  const hasThemeProvider = files.some(f =>
    f.language === "tsx" && /ThemeProvider/.test(f.content)
  );

  if (!hasThemeProvider) {
    violations.push({
      file: files[0]?.path ?? "unknown",
      message: "No <ThemeProvider> found — component tree must be wrapped in ThemeProvider",
      suggestion: "Import ThemeProvider and wrap the root component: <ThemeProvider theme={theme}>...</ThemeProvider>",
    });
  }

  return {
    id: "theme_provider_scope",
    score: hasThemeProvider ? 1.0 : 0.0,
